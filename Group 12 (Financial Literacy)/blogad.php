    ...
    <section class="search_bar">
        <form action="" class="container search_bar-container" action="">
            <div>
                ...
                <input type="search" name="" placeholder="search" id="filter">
                <button type="submit" class="btn" id="searchBtn">Go</button>
            </div>
        </form>
    </section>
    ...
    <script>
        ...
        $(document).ready(function() {
          $("#searchBtn").on("click", function(event) {
            ...
            var value = $("#filter").val().toLowerCase(); // Assuming the input field has the ID "filter"
        
            $(".post").each(function() {
              var postText = $(this).text().toLowerCase();
        
              if (postText.indexOf(value) > -1) {
                $(this).show();
              } else {
                $(this).hide();
                // const error = document.createElement("h2");
                // box.value = "No search found";
